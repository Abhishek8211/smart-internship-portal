...
const createDocumentId = (prefix: string) => `${prefix}_${randomUUID()}`;
...
export const dbStore = {
  ...
  profiles: {
    ...
    updateByUserId: async (userId: string, data: any) => {
      return await StudentProfile.findOneAndUpdate({ user: userId }, data, {
        new: true,
        upsert: true,
      });
    },
    ...
    addMessage: async (
      ...
      return await Chat.findOneAndUpdate(
        ...
        { $push: { messages: newMessage } },
        {
          ...
          setOnInsert: { _id: createDocumentId("chat") },
        },
      );
    },
  },
