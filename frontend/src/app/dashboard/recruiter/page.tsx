                ...
                >
                  ...
                  <div className="space-y-4">
                    {applicants.map((app: any, applicantIndex: number) => (
                      <div key={app._id || `${app.studentId || app.studentEmail || 'applicant'}-${app.internshipId || applicantIndex}`} className="p-6 glass-panel rounded-2xl border border-white/5 flex flex-col justify-between space-y-4 relative overflow-hidden glass-panel-hover">
                        ...
                        <div className="p-4 bg-slate-900/60 rounded-xl border border-white/5 grid sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-400">
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {app.studentProfile?.skills?.map((sk: string, skillIndex: number) => (
                                <span key={`${sk}-${skillIndex}`} className="px-2 py-0.5 rounded bg-white/5 text-slate-300 text-[10px] font-semibold">{sk}</span>
                              ))}
                            </div>
                          </div>
                          <div>
