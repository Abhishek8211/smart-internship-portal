...
export default function StudentDashboard() {
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      const localBms = JSON.parse(
        localStorage.getItem("local_bookmarks") || "[]",
      );
      setLocalBookmarks(localBms);
      const localApps = JSON.parse(
        localStorage.getItem("local_applications") || "[]",
      );
  ...
  const [activeTab, setActiveTab] = useState<
    "recommendations" | "applications" | "profile" | "chatbot"
  >("recommendations");
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const profileData = authDemoMode
        ? null
        : await apiFetch("/student/profile").catch(() => null);
      const jobsData: any[] = [];
      const storedApps = JSON.parse(
        localStorage.getItem("local_applications") || "[]",
      );
      const applicationsData = storedApps.map((id: string) => ({
        internshipId: id,
        status: "Applied",
      }));
      const notificationsData: any[] = [];
      const chatbotHistory: any[] = [];
      ...
      if (profileData) {
        ...
        setProfileLinks(
          profileData.links || { github: "", linkedin: "", portfolio: "" },
        );
        ...
        setProfile({
          ...
          applications: storedApps.map((id: string) => ({
            internshipId: id,
            status: "Applied",
          })),
          education: [
          ...
          : [
              {
                ...
                deadline: "In 5 days",
                description:
                  "Collaborate with the Stripe Dashboard UI team to build premium, responsive web components.",
                matchExplanation:
                  "Matches 4 out of 4 skills: React, TypeScript, Tailwind CSS, Redux Toolkit.",
              },
              {
                id: "intern_vercel_fs",
                ...
                deadline: "In 3 days",
                description:
                  "Build and deploy Server Actions, optimizing initial load performance on Next.js 15 apps.",
                matchExplanation: "Matches Next.js and TypeScript.",
              },
              {
                id: "intern_google_ai",
                ...
                deadline: "In 10 days",
                description:
                  "Train deep learning models for NLP pipelines, deploying inference servers in containerized workspaces.",
                matchExplanation: "Missing PyTorch and TensorFlow.",
              },
              {
                id: "intern_meta_cyber",
                ...
                deadline: "In 7 days",
                description:
                  "Audit cloud infrastructure access controls and build automated intrusion warning triggers.",
                matchExplanation: "Matches Linux and Python.",
              },
              {
                id: "intern_adobe_ui",
                ...
                deadline: "In 12 days",
                description:
                  "Design custom icons, flows, and interactive layouts for Photoshop web-release integrations.",
                matchExplanation: "Matches Figma prototyping.",
              },
            ];

      // Sort jobs by Match Score descending
      const sortedJobs = [...finalJobs].sort(
        (a, b) => (b.matchPercentage || 0) - (a.matchPercentage || 0),
      );
      setJobs(sortedJobs);
      setIsDemoMode(!Array.isArray(jobsData) || jobsData.length === 0);

      setApplications(applicationsData);
      setNotifications(notificationsData);
      setChatbotLog(chatbotHistory);
    } catch (e) {
      console.error(
        "Failed to load server dashboard data, falling back to local offline mode.",
        e,
      );
    } finally {
      setLoading(false);
  ...
  const handleApply = async (jobId: string) => {
    ...
    try {
      ...
      if (!localApps.includes(jobId)) localApps.push(jobId);
      localStorage.setItem("local_applications", JSON.stringify(localApps));
      setAppliedJobsLocally((prev) =>
        prev.includes(jobId) ? prev : [...prev, jobId],
      );
      setApplications((prev) =>
        prev.some((app) => app.internshipId === jobId)
          ? prev
          : [...prev, { internshipId: jobId, status: "Applied" }],
      );
      setProfile((prev: any) => ({
        ...prev,
        applications: [
          ...(prev?.applications || []),
          { internshipId: jobId, status: "Applied" },
        ],
      }));
      addToast(
  ...
  const handleBookmark = async (jobId: string, isSaved: boolean) => {
    setAnimatingStarId(jobId);
    const nextBookmarks = isSaved
      ? localBookmarks.filter((id) => id !== jobId)
      : [...localBookmarks, jobId];
    localStorage.setItem("local_bookmarks", JSON.stringify(nextBookmarks));
    setLocalBookmarks(nextBookmarks);
    setProfile((prev: any) => ({ ...prev, savedInternships: nextBookmarks }));
    addToast(
      "Demo Mode",
      isSaved ? "Job removed from saved jobs." : "Job saved locally.",
      "info",
    );
    window.setTimeout(() => setAnimatingStarId(null), 350);
  };
  ...
  const handleChatSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        let reply =
          "I can guide you on formatting: 1. Keep layout clean. 2. Mention metric highlights. 3. Add projects details.";
        if (userMsg.text.toLowerCase().includes("react")) {
          reply =
            "For React roles, learn: React 19 Actions, server rendering in Next.js, and state managers like Redux or Zustand.";
        }
        setChatbotLog((prev) => [
          ...prev,
  ...
  const handleSkillGapTrigger = async (job: any) => {
    ...
    try {
      ...
      const known = required.filter((s: string) =>
        mySkills.some((ms: string) => ms.toLowerCase() === s.toLowerCase()),
      );
      ...
      setGapAnalysis({
        ...
        roadmap: missing.map(
          (s: string) =>
            `Learn ${s} basics. Create a dashboard utilizing ${s}. Deploy a stack containing ${s}.`,
        ),
        courses: missing.map(
          (s: string) => `Complete guide to ${s} on Coursera/Udemy`,
        ),
      });
    }
  };
  ...
  const handleResumeSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addToast(
        "ATS Score",
        `Resume parsed! ATS Score: ${res.atsScore}/100. Profile updated.`,
        "success",
      );
    } catch (err) {
      const dummyProfile = {
        ...
        atsSuggestions: [
          ...
          "Add Docker and Postgres to your skill list.",
        ],
        skills: Array.from(
          new Set([
            ...(profile?.skills || []),
      ...
      addToast(
        "ATS Score",
        "Resume analyzed locally. ATS Score: 92/100. Profile synced.",
        "info",
      );
    } finally {
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/5 space-y-4">
            ...
            {profile?.atsSuggestions?.length > 0 && (
              <div className="space-y-2 pt-2 text-[10px] text-slate-400 font-semibold border-t border-white/5">
                <span className="text-white block font-bold text-[11px] mb-1">
                  AI Tuning Checklist:
                </span>
                {profile.atsSuggestions.map((sug: string, i: number) => (
                  <div
                    key={i}
                    className="flex space-x-2 items-start leading-relaxed"
                  >
              ...
              {activeTab === "recommendations" && (
                ...
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-semibold mt-1">
                        Smart candidate similarity ranking matching your active
                        profile
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="p-4 bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs rounded-2xl flex items-start space-x-3 leading-relaxed">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-slate-300 mt-0.5">
                          Running in offline demo mode. You can continue
                          exploring internships and saving applications locally.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    {jobs.map((job: any) => {
                      const isSaved = profile?.savedInternships?.includes(
                        job.id,
                      );
                      return (
                        <div
                          key={job.id}
                          className="glass-panel flex flex-col justify-between min-h-[480px] relative overflow-hidden group hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-300 rounded-2xl border border-white/5"
                        >
                          ...
                          <div>
                            ...
                            <div className="relative w-full h-40 overflow-hidden rounded-t-2xl">
                              ...
                              {(() => {
                                ...
                                return (
                                  <span
                                    className={`absolute top-3 right-3 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border flex items-center space-x-1 backdrop-blur-md ${badge.color}`}
                                  >
                            ...
                            <div className="p-5">
                              <div className="flex justify-between items-start gap-2 mb-3">
                                ...
                                <button
                                  onClick={() =>
                                    handleBookmark(job.id, isSaved)
                                  }
                                  className={`p-1.5 rounded-lg border transition-all duration-300 shrink-0 ${
                                    isSaved
                                      ? "bg-indigo-500/20 border-indigo-500 text-indigo-400"
                                      : "border-white/10 hover:border-indigo-500 text-slate-400 hover:text-white"
                                  } ${animatingStarId === job.id ? "scale-125" : ""}`}
                                >
                                  <Star
                                    className={`w-4 h-4 fill-current ${animatingStarId === job.id ? "animate-spin" : ""}`}
                                  />
                                </button>
                              </div>
                              ...
                              <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed mb-3">
                                {job.description ||
                                  "Exciting internship opportunity to join our engineering and product teams working on modern features."}
                              </p>
                              ...
                              <div className="flex flex-wrap gap-1.5 mb-4">
                                {job.skills_required
                                  ...
                                  .map((sk: string) => (
                                    <span
                                      key={sk}
                                      className="px-2 py-0.5 rounded text-[8px] font-bold bg-white/5 border border-white/5 text-slate-300"
                                    >
                                      {sk}
                                    </span>
                                  ))}
                            ...
                            {(() => {
                              const isApplied =
                                profile?.applications?.some(
                                  (app: any) => app.internshipId === job.id,
                                ) || appliedJobsLocally.includes(job.id);
                              const isApplying = applyingJobId === job.id;
                              return (
                                <button
                                  onClick={() =>
                                    !isApplied && handleApply(job.id)
                                  }
                  ...
                  {selectedGapJob &&
                    ...
                    (() => {
                      ...
                      const strokeDashoffset =
                        circumference - (matchPercent / 100) * circumference;
                      ...
                      const breakdown = [
                        ...
                        },
                        {
                          ...
                          value: Math.min(100, Math.round(matchPercent * 0.95)),
                        ...
                        },
                        {
                          label: "Academic CGPA",
                          value: profile?.cgpa
                            ? Math.min(100, Math.round(profile.cgpa * 25))
                            : 85,
                          color: "text-teal-400",
                          bg: "bg-teal-500/10",
                        },
                        {
                          label: "Projects Vetting",
                          value: profile?.projects?.length
                            ? Math.min(100, profile.projects.length * 40)
                            : 70,
                        ...
                        },
                        {
                          ...
                          value: profile?.certifications?.length
                            ? Math.min(100, profile.certifications.length * 50)
                            : 65,
                          color: "text-cyan-400",
                          bg: "bg-cyan-500/10",
                        },
                          ...
                          <div className="flex justify-between items-center border-b border-white/5 pb-3">
                            <div>
                              <h4 className="font-extrabold text-base text-white">
                                AI Match Analytics: {selectedGapJob.role}
                              </h4>
                              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mt-0.5">
                                {selectedGapJob.company_name} Vetting Parameters
                              </p>
                            </div>
                            <button
                              onClick={() => {
                                setSelectedGapJob(null);
                                setGapAnalysis(null);
                              }}
                              ...
                              <div className="flex flex-col sm:flex-row items-center gap-6 p-4 bg-slate-900/40 rounded-2xl border border-white/5">
                                ...
                                <div className="space-y-1">
                                  <h5 className="font-extrabold text-sm text-white">
                                    Vocation Match Score
                                  </h5>
                                  <p className="text-[10px] text-slate-400 font-semibold leading-relaxed">
                                    Evaluated across 7 key vetting parameters
                                    from your synced MongoDB and Prisma
                                    databases.
                                  </p>
                                </div>
                              </div>
                            ...
                            <div className="md:col-span-6 space-y-6">
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {gapAnalysis.known?.length > 0 ? (
                                      gapAnalysis.known.map((s: string) => (
                                        <span
                                          key={s}
                                          className="px-2.5 py-0.5 rounded-lg bg-teal-500/10 border border-teal-500/30 text-teal-300 text-[9px] font-black uppercase tracking-wider"
                                        >
                                          {s}
                                        </span>
                                      ))
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {gapAnalysis.missing?.length > 0 ? (
                                      gapAnalysis.missing.map((s: string) => (
                                        <span
                                          key={s}
                                          className="px-2.5 py-0.5 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-[9px] font-black uppercase tracking-wider"
                                        >
                                          {s}
                                        </span>
                                      ))
                                    ) : (
                                      <span className="text-xs text-teal-400 font-extrabold uppercase">
                                        100% Match!
                                      </span>
                                    )}
                                  </div>
                              ...
                              <div className="p-4 bg-slate-950/40 rounded-2xl border border-white/5 space-y-3.5">
                                ...
                                <div className="space-y-3 text-xs text-slate-300 font-semibold leading-relaxed">
                                  {gapAnalysis.roadmap?.map(
                                    (rm: string, idx: number) => (
                                      <div
                                        key={idx}
                                ...
                                <div className="pt-3 border-t border-white/5 space-y-2">
                                  ...
                                  {gapAnalysis.courses?.map(
                                    (c: string, idx: number) => (
                                      <div
                                        key={idx}
                                        className="flex items-center space-x-2 text-indigo-400 text-[11px] font-bold hover:underline cursor-pointer"
                                      >
              ...
              {activeTab === "applications" && (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-semibold mt-1">
                      Review active pipeline status checkpoints and recruiter
                      actions
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    {applications.map((app: any) => (
                      <div
                        key={app._id}
                        className="p-6 glass-panel rounded-2xl border border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
                      >
                        <div>
                          ...
                          <div className="flex space-x-4 text-xs text-slate-400 mt-3 font-semibold">
                            <span className="flex items-center space-x-1">
                              ...
                              <span>
                                {app.matchPercentage}% Compatibility Match
                              </span>
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:items-end gap-2 w-full sm:w-auto">
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-extrabold text-center border ${
                              app.status === "Selected"
                    ...
                    >
                      <div className="flex justify-between items-center border-b border-white/5 pb-3">
                        <h4 className="font-extrabold text-sm text-amber-400 flex items-center space-x-2">
                          <BrainCircuit className="w-5 h-5" />
                          <span>
                            AI practice interview sheets generated for{" "}
                            {activeInterviewApp.internship?.role}
                          </span>
                        </h4>
                        <button
                      ...
                      <div className="p-4 bg-amber-500/5 rounded-xl border border-amber-500/10 text-xs text-slate-400 leading-relaxed font-semibold">
                        <p className="mb-2 text-white">
                          📅 **Schedule details**:{" "}
                          {activeInterviewApp.interviewDetails.dateTime}
                        </p>
                        <p className="mb-2 text-indigo-400">
                          🔗 **Meeting Link**:{" "}
                          <a
                            href={activeInterviewApp.interviewDetails.link}
                            target="_blank"
                            rel="noreferrer"
                            className="underline"
                          >
                            {activeInterviewApp.interviewDetails.link}
                          </a>
                        </p>
                        <p>
                          📝 **Recruiter Notes**:{" "}
                          {activeInterviewApp.interviewDetails.notes}
                        </p>
                      </div>
                      ...
                      {activeInterviewApp.interviewDetails.aiQuestions && (
                        <div className="grid sm:grid-cols-2 gap-6 pt-2">
                          <div className="space-y-4">
                            <div>
                              <span className="text-white block font-bold text-xs uppercase tracking-wider mb-2">
                                Technical Questions:
                              </span>
                              <ul className="list-decimal pl-4 text-xs text-slate-400 font-semibold space-y-2 leading-relaxed">
                                {activeInterviewApp.interviewDetails.aiQuestions.technical?.map(
                                  (q: string, i: number) => (
                                    <li key={i}>{q}</li>
                                  ),
                                )}
                            ...
                            <div>
                              ...
                              <ul className="list-decimal pl-4 text-xs text-slate-400 font-semibold space-y-2 leading-relaxed">
                                {activeInterviewApp.interviewDetails.aiQuestions.coding?.map(
                                  (q: string, i: number) => (
                                    <li key={i}>
                                      <code>{q}</code>
                                    </li>
                          ...
                          <div className="space-y-4">
                            <div>
                              ...
                              <ul className="list-decimal pl-4 text-xs text-slate-400 font-semibold space-y-2 leading-relaxed">
                                {activeInterviewApp.interviewDetails.aiQuestions.behavioral?.map(
                                  (q: string, i: number) => (
                                    <li key={i}>{q}</li>
                                  ),
                                )}
                            ...
                            <div>
                              ...
                              <ul className="list-decimal pl-4 text-xs text-slate-400 font-semibold space-y-2 leading-relaxed">
                                {activeInterviewApp.interviewDetails.aiQuestions.hr?.map(
                                  (q: string, i: number) => (
                                    <li key={i}>{q}</li>
                                  ),
                                )}
                ...
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-semibold mt-1">
                        Manage your professional identity and application
                        preferences
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6 lg:col-span-1">
                      ...
                      <div className="p-6 glass-panel rounded-2xl border border-white/5 space-y-6 relative overflow-hidden">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400">
                            Import your credentials from external platforms to
                        ...
                        <div className="space-y-4">
                          <form
                            ...
                            className="space-y-3 p-4 bg-slate-900/50 rounded-xl border border-white/5"
                          >
                      ...
                      <div className="p-6 glass-panel rounded-2xl border border-white/5 space-y-4">
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400">
                            Manage your portal preferences and account security.
                          </p>
                        </div>
                    ...
                    <div className="lg:col-span-2">
                      <form
                        ...
                        className="p-8 glass-panel rounded-2xl border border-white/5 space-y-8 relative overflow-hidden"
                      >
                          ...
                          {profileEducation.length === 0 ? (
                            ...
                            <div className="space-y-3">
                              {profileEducation.map((edu, idx) => (
                                <div
                                  key={idx}
                                  className="flex justify-between items-center bg-slate-900 p-4 rounded-xl border border-white/5 shadow-sm group"
                                >
                                  <div>
                          ...
                          <div className="bg-slate-950 p-5 rounded-xl border border-indigo-500/20 shadow-inner mt-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                              <input
                                ...
                                className="px-3 py-2 rounded-lg bg-slate-900 border border-white/10 text-xs text-white focus:border-indigo-500 focus:outline-none"
                              />
                              <input
                                type="text"
                                ...
                                value={newEduDegree}
                                onChange={(e) =>
                                  setNewEduDegree(e.target.value)
                                }
                                className="px-3 py-2 rounded-lg bg-slate-900 border border-white/10 text-xs text-white focus:border-indigo-500 focus:outline-none"
                              />
                              <input
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg text-xs font-bold transition-all shadow-md"
                            >
                              Add Education
                            </button>
                          </div>
                        ...
                        <div className="space-y-4 relative z-10">
                          ...
                          {profileProjects.length === 0 ? (
                            <div className="p-6 bg-slate-900/50 rounded-xl border border-white/5 text-center border-dashed">
                              ...
                              <p className="text-xs text-slate-400 font-semibold">
                                No projects added yet. Boost your ATS score by
                                adding some!
                              </p>
                            </div>
                          ) : (
                            <div className="space-y-3">
                              {profileProjects.map((proj, idx) => (
                                <div
                                  key={idx}
                                  className="flex justify-between items-start bg-slate-900 p-4 rounded-xl border border-white/5 shadow-sm group"
                                >
                                  <div className="space-y-1.5">
                                    <div className="flex items-center space-x-2">
                                      <p className="text-sm font-extrabold text-white">
                                    ...
                                    <div className="flex flex-wrap gap-1.5 pt-1">
                                      {Array.isArray(proj.technologies) ? (
                                        proj.technologies.map((t: string) => (
                                          <span
                                            key={t}
                                            className="px-2 py-0.5 rounded-md bg-white/5 text-slate-300 text-[9px] font-bold uppercase tracking-wider"
                                          >
                                            {t}
                                          </span>
                                        ))
                                      ) : (
                                        <span className="px-2 py-0.5 rounded-md bg-white/5 text-slate-300 text-[9px] font-bold uppercase tracking-wider">
                                          {proj.technologies}
                                        </span>
                                      )}
                                    </div>
                          ...
                          <div className="bg-slate-950 p-5 rounded-xl border border-teal-500/20 shadow-inner mt-4">
                            <p className="text-[10px] font-bold uppercase tracking-wider text-teal-400 mb-3">
                              Add New Project
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                              <input
                                ...
                                className="px-3 py-2 rounded-lg bg-slate-900 border border-white/10 text-xs text-white focus:border-teal-500 focus:outline-none"
                              />
                              <input
                                type="text"
                              ...
                              <input
                                ...
                                className="px-3 py-2 rounded-lg bg-slate-900 border border-white/10 text-xs text-white focus:border-teal-500 focus:outline-none md:col-span-2"
                              />
                              <textarea
                                ...
                                rows={3}
                                className="px-3 py-2 rounded-lg bg-slate-900 border border-white/10 text-xs text-white focus:border-teal-500 focus:outline-none md:col-span-2 resize-none"
                              />
                            </div>
                            <button
                              type="button"
                              onClick={handleAddProject}
                              className="px-4 py-2 bg-teal-500 hover:bg-teal-600 text-white rounded-lg text-xs font-bold transition-all shadow-md"
                            >
                              Add Project
                            </button>
                          </div>
                        ...
                        <div className="space-y-4 relative z-10">
                          ...
                          {profileCertifications.length === 0 ? (
                            <div className="p-6 bg-slate-900/50 rounded-xl border border-white/5 text-center border-dashed">
                              ...
                              <p className="text-xs text-slate-400 font-semibold">
                                No certifications added. Stand out by adding
                                some!
                              </p>
                            </div>
                          ) : (
                          ...
                          <div className="bg-slate-950 p-5 rounded-xl border border-indigo-500/20 shadow-inner mt-4">
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-indigo-500/20 hover:bg-indigo-500 text-indigo-400 hover:text-white rounded-lg text-xs font-bold transition-all shadow-md"
                            >
                              Add Certificate
                            </button>
                          </div>
              ...
              {activeTab === "chatbot" && (
                ...
                >
                  <div className="px-6 py-4 border-b border-indigo-500/10 bg-indigo-500/5 flex items-center space-x-2">
                    ...
                    <div>
                      <h4 className="font-bold text-sm">
                        AI Career Counselor Sandbox
                      </h4>
                      <p className="text-[10px] text-slate-400 font-semibold">
                        Tuned to evaluate matching algorithms against your
                        skills profile
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex-grow p-6 overflow-y-auto space-y-4">
                    {chatbotLog.map((msg: any, i: number) => (
                      ...
                      >
                        ...
                        <div
                          className={`p-3.5 rounded-2xl text-xs leading-relaxed font-semibold whitespace-pre-wrap shadow-md ${
                            msg.sender === "user"
                  ...
                  <form
                    ...
                    className="p-4 border-t border-indigo-500/10 bg-slate-950 flex gap-2"
                  >
                    <input
                      type="text"
              ...
              <div className="p-4 rounded-xl bg-slate-950 border border-white/5 space-y-2.5 text-xs font-semibold text-slate-300">
                <p className="flex justify-between">
                  ...
                  <span className="text-white">
                    {applyConfirmJob.company_name}
                  </span>
                </p>
                <p className="flex justify-between">
                  <span className="text-slate-400">Location:</span>{" "}
                  <span className="text-white">{applyConfirmJob.location}</span>
                </p>
                <p className="flex justify-between">
                  <span className="text-slate-400">Salary:</span>{" "}
                  <span className="text-purple-400 font-extrabold">
                    {applyConfirmJob.stipend}
                  </span>
                </p>
              </div>
            </div>
