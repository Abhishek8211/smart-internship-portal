...
export default function StudentDashboard() {
  const { user, logout, apiFetch, loading: authLoading, isDemoMode: authDemoMode } = useAuth();
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const profileData = authDemoMode ? null : await apiFetch('/student/profile').catch(() => null);
      const jobsData: any[] = [];
      const storedApps = JSON.parse(localStorage.getItem('local_applications') || '[]');
      const applicationsData = storedApps.map((id: string) => ({ internshipId: id, status: 'Applied' }));
      ...
      const finalJobs = Array.isArray(jobsData) && jobsData.length > 0 ? jobsData : [
  ...
  const handleApply = async (jobId: string) => {
    ...
    try {
      ...
      setAppliedJobsLocally(prev => prev.includes(jobId) ? prev : [...prev, jobId]);
      setApplications(prev => prev.some(app => app.internshipId === jobId) ? prev : [...prev, { internshipId: jobId, status: 'Applied' }]);
      setProfile((prev: any) => ({
        ...prev,
        applications: [...(prev?.applications || []), { internshipId: jobId, status: 'Applied' }]
      }));
      addToast('Application Submitted', 'Your application was sent successfully.', 'success');
    } finally {
  ...
  const handleBookmark = async (jobId: string, isSaved: boolean) => {
    ...
    addToast('Demo Mode', isSaved ? 'Job removed from saved jobs.' : 'Job saved locally.', 'info');
  ...
  const handleResumeSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const dummyProfile = {
        ...
        atsScore: 92,
        atsSuggestions: ['Quantify project metrics.', 'Add Docker and Postgres to your skill list.'],
        skills: Array.from(new Set([...(profile?.skills || []), 'React', 'Node.js', 'PostgreSQL', 'Docker']))
      };
      setProfile(dummyProfile);
      addToast('ATS Score', 'Resume analyzed locally. ATS Score: 92/100. Profile synced.', 'info');
    } finally {
